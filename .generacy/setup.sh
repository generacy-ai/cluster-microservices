...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Configures the fork/clone layout only. Clusters provisioned by the"
            echo "generacy CLI or generacy-cloud are configured by those tools instead."
            exit 0
            ;;
...
#
# setup.sh configures the *fork/clone* layout, where cluster-base's own compose
...
# True if this repo was provisioned by the generacy CLI / generacy-cloud rather
# than forked from cluster-base. Any one marker is sufficient.
looks_cli_or_cloud_provisioned() {
    [ -f "${GENERACY_DIR}/cluster-base.json" ] && return 0
    if [ -f "${DEVCONTAINER_DIR}/.env" ] && \
       head -n 5 "${DEVCONTAINER_DIR}/.env" 2>/dev/null | grep -q 'Generated by Generacy'; then
        return 0
    fi
    if [ -f "$CONFIG_FILE" ] && grep -q 'generacy#247' "$CONFIG_FILE" 2>/dev/null; then
        return 0
    fi
    return 1
}
...
ENV_FILE=""
if [ -f "${GENERACY_DIR}/docker-compose.yml" ]; then
    # Only the generacy CLI scaffolds a compose into .generacy/.
    CLUSTER_LAYOUT="cli"
    ENV_FILE="${GENERACY_DIR}/.env"
elif [ -f "${DEVCONTAINER_DIR}/docker-compose.yml" ]; then
    if looks_cli_or_cloud_provisioned; then
...
if [ "$CLUSTER_LAYOUT" != "fork" ]; then
    error "This doesn't look like a fork/clone of cluster-base — refusing to write a no-op."
    echo ""
    case "$CLUSTER_LAYOUT" in
        cli)
            echo "  This cluster is governed by .generacy/docker-compose.yml, which reads"
            echo "  .generacy/.env — not .devcontainer/generacy/.env. That file is owned by"
            echo "  the 'generacy' CLI and carries identity values this script must not"
            echo "  overwrite."
            echo ""
            echo "  To set the smee webhook channel, edit SMEE_CHANNEL_URL in:"
            echo "    ${ENV_FILE}"
            echo "  (leave the identity block above it untouched) and restart the cluster."
            ;;
        cloud)
            echo "  This repo was provisioned by generacy-cloud. Its"
            echo "  .devcontainer/generacy/.env is a decoy that no container loads — the"
            echo "  real cluster runs its compose from a separate cluster directory."
            echo ""
            echo "  Configure this cluster through the 'generacy' CLI or your generacy-cloud"
            echo "  project settings instead."
            ;;
        *)
            echo "  No docker-compose.yml was found governing this cluster (looked in"
            echo "  .generacy/ and .devcontainer/generacy/). If this is a fork, run the"
            echo "  script from the repo root; otherwise configure the cluster through the"
            echo "  tool that provisioned it."
            ;;
    esac
    echo ""
    echo "  Webhook forwarding is moving to cluster-local auto-provisioning"
    echo "  (generacy-ai/generacy#952), which removes the need to run this script for"
    echo "  smee on CLI/cloud clusters."
    echo ""
    exit 1
fi

ok "Cluster layout: fork/clone — configuring ${ENV_FILE}"
...
# Check if smee is already configured in the env file the compose actually reads
# (resolved in Step 0). Reading the right file is what stops a re-run from
# minting a fresh smee channel every time. See cluster-base#81.
SMEE_CHANNEL_URL=""
if [ -f "$ENV_FILE" ]; then
