...
$SmeeChannelUrl = ""
$EnvFile = Join-Path $DevcontainerDir "project.env"
...
if ($SmeeChannelUrl) {
    Write-Ok "Smee channel (from existing project.env): $SmeeChannelUrl"
} else {
    ...
    if (-not $SmeeChannelUrl) {
        ...
        if ($SmeeChannelUrl) {
            ...
            Write-Warn "Skipping smee.io - you can add SMEE_CHANNEL_URL to project.env later"
        }
    }
}
...
Write-Info "Generating .devcontainer/project.env..."

$writeEnv = $true
if (Test-Path $EnvFile) {
    $overwrite = Read-Host "  ? project.env already exists. Overwrite? [y/N]"
    if ($overwrite -ne "y" -and $overwrite -ne "Y") {
        Write-Warn "Keeping existing project.env"
        $writeEnv = $false
    }
}
...
Write-Host "    .devcontainer/project.env   - Docker Compose settings (commit this)"
